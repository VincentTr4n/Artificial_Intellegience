...
public class Solution {
	...
	public static int cache[][] = { {1, 1}, {0, 0}, {0, 1}, {0, 2}, {1, 2}, {2, 2}, {2, 1}, {2, 0}, {1, 0} };
	//
	// Mảng toán tử tương ứng với các thao tác di chuyển : right, left, down, up
	public static int dx[] = { 1, -1, 0 , 0 };
	public static int dy[] = { 0, 0, 1 , -1 };
	//public static StdIn in = new StdIn("src/BaiToan8So/input_8so.txt");
	...
	public static void breadth_first_search(State begin, State end) {
		...
		while(!queue.isEmpty()) {
			...
			if(cur.equals(end)) {
				...
				System.out.println("Time : " + (System.currentTimeMillis() - s) / 1000d + "s");
				return;
			}
	...
	public static void hill_climb(State begin, State end) {
		...
		while(!st.isEmpty()) {
			...
			for(MyData item : list) st.push(item.state);
		}
		
	}

	public static void main(String[] args) throws Exception {
//		int a1[][] = in.readIntMatrix(3, 3);
//		int a2[][] = in.readIntMatrix(3, 3);
		int a1[][] = { { 2, 8, 3 },
					   { 1, 6, 4 },
					   { 7, 0, 5} };
	...
	public static void printSolution(State cur, HashMap<State, State> map) {
    	...
    	// Gọi đệ quy tin tiếp các trạng thái còn lại
    	System.out.println(cur.toString());
    }
	
	// Lớp đối tượng để phục vụ cho việc so sánh trong hàm đợi ưu tiên
	public static class MyData implements Comparable<MyData>{
    	public int h;
    	public State state;
    	public MyData() {}
    	public MyData(State c) {
    		state = c;
